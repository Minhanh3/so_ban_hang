  ...
  return (
    <div className="max-w-6xl mx-auto space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex gap-2">
          ...
          <button
            ...
            className="bg-primary-light text-primary px-3 py-2.5 rounded-xl font-bold flex items-center gap-2 hover:opacity-90 transition-all border border-primary-light"
            title="Xuất file"
          >
          ...
          <button
            ...
            className="bg-primary text-white px-5 py-2.5 rounded-xl font-semibold flex items-center gap-2 hover:opacity-90 transition-all shadow-lg shadow-primary-light"
          >
            <Plus size={20} />
            Ghi nợ mới
          </button>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="bg-primary-light border border-primary-light p-5 rounded-2xl flex items-center justify-between">
          <div>
            <span className="text-xs font-bold text-primary uppercase">Khách nợ mình</span>
            <p className="text-2xl font-bold text-primary">{totalReceivable.toLocaleString()}đ</p>
          </div>
          <DollarSign className="opacity-20 text-primary" size={40} />
        </div>
        ...
        {filteredDebts.map(debt => (
          <div key={debt.id} className="p-4 hover:bg-gray-50 flex items-center justify-between border-b last:border-0 group">
            ...
            <div className="text-right">
              <p className={`font-bold text-lg ${debt.type === 'customer_receivable' ? 'text-primary' : 'text-red-600'}`}>
